...
**操作步骤：**
1. 提取参考文章内容（按优先级尝试以下方式）：
   - **方式 A（推荐）**：使用智能体内置的 `web_fetch` 工具直接读取 URL 内容
   - **方式 B（备选）**：使用 `scripts/extract_to_markdown.py` 脚本提取
   - **方式 C（兜底）**：如果以上方式均失败（常见原因：微信反爬验证码），提示用户直接粘贴文章内容，转入模式 4
2. 智能体改写文章内容
...
**⚠️ 提取失败处理：**
微信公众号文章有反爬机制，云端服务器 IP 容易被拦截。当提取失败时，智能体应**立即提示用户**：
```
❌ 无法直接提取文章内容（微信反爬保护）。
请您将文章内容复制粘贴发给我，我来帮您改写。
操作方法：在微信中打开文章 → 全选复制文字 → 粘贴发送给我
```
...
智能体: 
1. 尝试读取文章内容...
   （优先 web_fetch，失败则尝试 extract_to_markdown.py）
2. 改写文章...
3. 生成封面...
4. 上传到草稿箱...
✅ 完成！草稿已创建，media_id: xxx
...
#### 模式 4：基于用户粘贴的文章内容改写（适用于云端环境）

用户直接粘贴文章文本内容，智能体改写后发布。**这是最可靠的方式，不受微信反爬限制。**

**操作步骤：**
1. 用户直接发送文章文本内容（复制粘贴）
2. 智能体分析内容，提取标题和正文
3. 智能体改写文章内容
4. 生成封面图片（使用 `generate_cover.py`）
5. 上传封面到微信（使用 `upload_material.py`）
6. 创建草稿（使用 `create_draft.py`）
...
微信公众号对云端服务器 IP 有反爬机制（验证码保护），因此需要按以下优先级尝试：

1. **策略一（推荐）：使用 `web_fetch` 内置工具**
   - 直接使用智能体内置的 `web_fetch` 工具读取文章 URL
   - 这是最简单直接的方式，通常能绕过部分反爬限制
   - 获取到的 HTML/文本内容由智能体自行转换为 Markdown
...
3. **策略三（兜底）：请用户粘贴文章内容**
   - 如果以上策略均因反爬限制失败，提示用户：
     ```
     ❌ 微信文章有反爬保护，无法自动提取。
     请您在微信中打开文章，复制全部文字内容后粘贴发给我。
     ```
   - 收到用户粘贴的内容后，智能体自行整理为 Markdown 格式
...
- 当 **URL提取失败**：使用流程 0 模式 4（用户粘贴文章内容改写）
...
- 发布操作不可逆，建议先创建草稿确认内容
- 注意API调用频率限制
- **微信反爬机制**：微信公众号文章对云端服务器有反爬保护（验证码），直接抓取 URL 可能失败。解决方案：
  1. 优先使用内置 `web_fetch` 工具读取 URL
  2. 若仍失败，引导用户复制粘贴文章内容（模式 4）
  3. 切勿反复重试失败的 URL 提取，应立即切换策略
...
### 触发方式
当用户说以下指令时，使用 LINHAO-style 排版：
- "使用 LINHAO-style 排版"
- "按照林浩风格排版"
- "用林浩公众号风格"

### 风格特点
- **正文**：16px 字体 / 28px 行高 / rgb(63,63,63) 深灰色
- **标题**：绿色背景 (#009874) + 白色文字 + 居中对齐 + 圆角
...
### 配置文件
详见 `LINHAO_STYLE.md` 获取完整样式规范和 HTML 模板。
