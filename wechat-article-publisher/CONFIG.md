...
- **默认提供商**: qwen（千问）
- **图片模型**: `qwen-image-2.0`
- **千问（DashScope）API Key**: `your_dashscope_api_key_here`

### 2.1 Tavily 搜索 API
- **Tavily API Key**: `your_tavily_api_key_here`
- **用途**: 联网搜索资料，用于基于标题创作文章时自动搜索背景材料

### 3. 文章排版

#### 3.1 排版工具
- **默认引擎**: doocs（doocs/md 排版引擎）
- **主题**: 根据文章内容动态选择（不固定）
  - 可用主题：orange, blue, green, purple, red, cyan, black, pink
  - 选择原则：根据文章类型、情绪、目标受众
  - 详见：`THEME_GUIDE.md`

#### 3.2 LINHAO-style 排版风格（林浩风格）

**风格说明**：专为林浩学 AI 编程公众号设计的排版风格，适合科技干货、工具教程类文章。

**配置文件**：`LINHAO_STYLE.md`

**风格特点**：
- 正文：16px 字体 / 28px 行高 / rgb(63,63,63) 颜色
- 标题：绿色背景 (#009874) + 白色文字 + 居中对齐
- 代码块：GitHub Dark 风格 + macOS 圆点 + 语法高亮
- 提示框：浅灰背景 + emoji 图标
- 分隔线：灰色细线
